...
<!-- Set Position Modal -->
<div class="modal fade" id="setPositionModal" tabindex="-1" aria-labelledby="setPositionModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content text-dark">
      <div class="modal-header">
        <h5 class="modal-title fw-bold text-dark" id="setPositionModalLabel">Set Position for <span id="modalUsername" class="text-primary"></span></h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <input type="hidden" id="modalUserId">
        <p class="mb-3 text-secondary">Choose the position for this member in the binary tree:</p>

        <div class="form-check mb-2">
          <input class="form-check-input" type="radio" name="positionOption" id="optionLeft" value="left">
          <label class="form-check-label fw-bold text-dark" for="optionLeft">
            Left
          </label>
        </div>

        <div class="form-check mb-2">
          <input class="form-check-input" type="radio" name="positionOption" id="optionRight" value="right">
          <label class="form-check-label fw-bold text-dark" for="optionRight">
            Right
          </label>
        </div>

        <div class="form-check mb-3">
          <input class="form-check-input" type="radio" name="positionOption" id="optionNone" value="none" checked>
          <label class="form-check-label fw-bold text-dark" for="optionNone">
            Not decided yet
          </label>
        </div>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" id="btnConfirmPosition">OK</button>
      </div>
    </div>
  </div>
</div>
...
<script type="text/javascript">
  $(document).ready(function() {
    // When "Set Postion" is clicked
    $(document).on('click', '.btn-set-position', function(e) {
      e.preventDefault();
      ...
      // Default choice is "Not decided yet" (none)
      $('#optionNone').prop('checked', true);

      // Open the bootstrap modal
      var setPosModal = new bootstrap.Modal(document.getElementById('setPositionModal'));
      setPosModal.show();
    });

    // When "OK" button inside the modal is clicked
    $('#btnConfirmPosition').on('click', function() {
      var userId = $('#modalUserId').val();
      var selectedPos = $('input[name="positionOption"]:checked').val();
      var confirmBtn = $(this);
      ...
      $.ajax({
        ...
        success: function(response) {
          confirmBtn.prop('disabled', false).text('OK');
          if (response.status === 'success') {
            // Hide the modal using bootstrap API
            var modalEl = document.getElementById('setPositionModal');
            var modalInstance = bootstrap.Modal.getInstance(modalEl);
            if (modalInstance) {
              modalInstance.hide();
            }
            // Reload the page to show the updated position
            location.reload();
          } else {
        ...
        error: function(xhr, status, error) {
          confirmBtn.prop('disabled', false).text('OK');
          $('#modalAlert').removeClass('d-none').text('An error occurred while setting the position: ' + error);
        }
      });
    });
  });
