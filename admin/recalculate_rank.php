...
// Helper function to get allowable amount under 300% cap (matching MLMEngine private helper)
...
if ($user) {
    $legStats = $engine->getLegsBusiness($user['id']);
        ...
        if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['admin_csrf']) {
            ...
            try {
                ...
                if ($allowable > 0) {
                    ...
                    $db->rollBack();
                    $recalcError = "Failed to pay. User has hit the 300% ID Cap limit.";
                }
            } catch (Exception $e) {
                if ($db->inTransaction()) {
                    $db->rollBack();
