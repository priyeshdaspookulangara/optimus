...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_genealogy') {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['admin_csrf']) {
        ...
        $newPosition = $_POST['position'] === '' ? null : $_POST['position'];
        ...
        try {
            ...
            if ($userId === 1) {
                // Only allow position change (though usually root has no position, but we can update it if specified)
            ...
            // Rebuild Genealogy Table to reflect changes
...
<div class="mb-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <span class="badge bg-secondary">Total Members: <?php echo count($users); ?></span>
    </div>

    <!-- Filters -->
    <form method="get" class="row g-2 align-items-center bg-white p-3 rounded border shadow-sm">
        <div class="col-md-5">
            <div class="input-group">
                <span class="input-group-text"><i class="fa fa-search text-muted"></i></span>
                <input type="text" name="search" class="form-control" placeholder="Search by username, email, MID..." value="<?php echo htmlspecialchars($search); ?>">
            </div>
        </div>
        <div class="col-md-4">
            <select name="position" class="form-select">
                <option value="">-- All Positions --</option>
                <option value="left" <?php echo $positionFilter === 'left' ? 'selected' : ''; ?>>Left</option>
                <option value="right" <?php echo $positionFilter === 'right' ? 'selected' : ''; ?>>Right</option>
