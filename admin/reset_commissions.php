    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['admin_csrf']) {
        ...
        if ($_POST['action'] == 'recalculate') {
            ...
            try {
                ...
                $txStmtReferrals = $db->prepare("SELECT COUNT(*) as ref_count FROM users WHERE sponsor_id = ?");
                ...
                foreach ($finalScheds as $sched) {
                    ...
                    for ($day = 1; $day <= $daysPassed; $day++) {
                        ...
                        if ($allowable > 0) {
                            ...
                            // Fetch and propagate up to sponsors
                            $txStmtUplines->execute([$schedUserId]);
                            $uplines = $txStmtUplines->fetchAll(PDO::FETCH_ASSOC);

                            $consecutiveSingleCount = 0;
                            foreach ($uplines as $upline) {
                                // Check if this parent has only one direct referral
                                $txStmtReferrals->execute([$upline['parent_id']]);
                                $refData = $txStmtReferrals->fetch(PDO::FETCH_ASSOC);
                ...
                $success_msg = ($success_msg ?: "System Recalculation Completed successfully!") . " All ranks, structures, level incomes, and matching schedules are perfectly synchronized.";
            } catch (Exception $e) {
                $db->rollBack();
...
if (isset($_GET['user_id'])) {
    ...
    $stmtSel = $db->prepare("SELECT u.*, r.name as rank_name FROM users u LEFT JOIN ranks r ON u.rank_id = r.id WHERE u.id = ?");
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                ...
                <ul class="small">
                    ...
                    <li>Re-process matching schedule durations and propagate matching daily payouts up the sponsors.</li>
                </ul>
